...
export default function Defaulters() {
  ...
  const defaulters = useMemo(() => {
    ...
    return plansData
      ...
      .map((p) => ({
        ...p,
        daysOverdue: daysBetween(today, toDate(p.nextDueDate)),
        amountDue: p.monthlyAmount || p.installmentAmount || p.fdAmount || 0
      }))
      .sort((a, b) => b.daysOverdue - a.daysOverdue)
  }, [plansData, scopeOwn, agentUid])
              ...
              <tbody>
                {defaulters.map((d) => (
                  <tr key={d.id}>
                    ...
                    <td className="text-ink-2">{d.type || d.planCode}</td>
                    ...
                    <td>
                      <button
                        type="button"
                        onClick={() => shareWhatsApp(reminderMessage({ name: d.customerName, amount: d.amountDue, planAccount: d.planAccountNumber || d.policyNumber, dueDate: d.nextDueDate, agentName: d.agentName }), null)}
